...
class start_goal_publisher:
    def __init__(self):
        ...
        self.queue_LS_pseudo_actual= np.zeros([2,4]) -1
        self.queue_LS_assigned = np.zeros([2,4])-1
            ...
            if(self.current_pose[i][1]<12):
                ...
                if(self.current_pose[i][1]>7.5):
                    if self.current_pose[i][0]<30:
                        self.queue_LS_actual[0][int((30-self.current_pose[i][0])//6)+1]=i
                        bot_found[0][int((30-self.current_pose[i][0])//6)+1]=1
                    elif self.current_pose[i][0]>54:
                        self.queue_LS_actual[1][int((self.current_pose[i][0]-54)//6)+1]=i
                        bot_found[1][int((self.current_pose[i][0]-54)//6)+1]=1
    ...
    def preffered_LS(self, bot_num):
        Loss_function_LS0 =self.lossfunction_para1 * sqrt((self.current_pose[bot_num][0] - self.LS_queue_locations[0][4][0])**2 + (self.current_pose[bot_num][1] - self.LS_queue_locations[0][4][1])**2) 
        Loss_function_LS1 =self.lossfunction_para1 * sqrt((self.current_pose[bot_num][0] - self.LS_queue_locations[1][4][0])**2 + (self.current_pose[bot_num][1] - self.LS_queue_locations[1][4][1])**2)
